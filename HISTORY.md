...
### apps/claude/CLAUDE.md
- 「サブエージェント委譲を原則とする」を改訂
- 委譲対象に「テスト実行・ビルド・lint・ログ解析など出力ボリュームが大きい実行系」を明記（従来は読み取り系のみだった）
- 判断基準を追加: 「`tail -N` や `head -N` で切り詰めたくなったら委譲する」（パイプ扱いルールとクロスリファレンス）
- 「直接実行してよい」を編集・コミット等の書き込み系も含む形に拡張し、判断ブレを抑制
