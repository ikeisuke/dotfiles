...
Fine-grained と Classic を別々のサービス名で保持できる:

| 種類 | サービス名 | 用途 |
|------|-----------|------|
| Classic | `ai-agent-gh-token-classic` | 全リポジトリアクセスが必要な場合 |
| Fine-grained | `ai-agent-gh-token-fine-grained` | リポジトリ限定アクセス（推奨） |

```bash
# Fine-grained PAT を保存
security add-generic-password \
  -s "ai-agent-gh-token-fine-grained" \
  -a "$USER" \
  -w "github_pat_xxxxxxxxxxxx"

# Classic PAT を保存（必要な場合）
security add-generic-password \
  -s "ai-agent-gh-token-classic" \
  -a "$USER" \
  -w "ghp_xxxxxxxxxxxxxxxxxxxx"
```

### Linux/WSL（GNOME Keyring）

```bash
# 依存パッケージのインストール
sudo apt install libsecret-tools gnome-keyring

# Fine-grained PAT を保存（echo -n で制御文字混入を防止）
echo -n "github_pat_xxxxxxxxxxxx" | secret-tool store \
  --label="GitHub PAT Fine-grained" \
  service "ai-agent-gh-token-fine-grained" \
  account "$USER"

# Classic PAT を保存（必要な場合）
echo -n "ghp_xxxxxxxxxxxxxxxxxxxx" | secret-tool store \
  --label="GitHub PAT Classic" \
  service "ai-agent-gh-token-classic" \
  account "$USER"
```
...
```bash
# トークンの値を表示
secret-tool lookup service "ai-agent-gh-token-fine-grained" account "$USER"

# 制御文字が混入していないか確認（末尾に 0a 等がないこと）
secret-tool lookup service "ai-agent-gh-token-fine-grained" account "$USER" | xxd | head -1
```

> **注意**: `secret-tool store` にパイプではなく対話入力でトークンを渡す場合、
> Enter キーの改行文字がトークンに含まれることがある。`echo -n` でパイプするのが安全。
...
```bash
GH_KEYCHAIN_SERVICE="ai-agent-gh-token-fine-grained"  # または ai-agent-gh-token-classic
```
...
```bash
# トークンが保存されているか確認
security find-generic-password -s "ai-agent-gh-token-fine-grained" -a "$USER"

# トークンの値を表示（確認用）
security find-generic-password -s "ai-agent-gh-token-fine-grained" -a "$USER" -w
```

## 4. トークンのローテーション

ローテーションスクリプトで一括管理（macOS / Linux 両対応）:

```bash
# 対話式で全トークンを順番にローテーション（有効期限も表示）
gh-token-rotate

# 種類を指定してローテーション
gh-token-rotate classic
gh-token-rotate fine-grained
```
